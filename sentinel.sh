...
# =============================================================================
# SAFE_MODE = 1  (RECOMMENDED)
#   Only unambiguous MINERS (by name / mining signature) are killed. The blanket
#   "any process >70% CPU" and ">50% memory" killers are DISABLED so your own
#   SSH / terminal / dev tools are NEVER touched just for using CPU.
#   The host still kills xmrig, minerd, ethminer, renamed /tmp miners, etc.
# SAFE_MODE = 0
#   Full aggressive mode: ALSO kills anything exceeding the CPU/mem thresholds,
#   and any unknown binary in /tmp,/dev/shm,~. Use only if you truly want to
#   police EVERY process and accept that heavy legit workloads get terminated.
# =============================================================================
SAFE_MODE=1

# Also disable the aggressive "kill any unknown binary in /tmp" heuristic under
# SAFE_MODE (it can hit user/dev tools). Forced off when SAFE_MODE=1.
...
check_miner_names() {
    ...
    while IFS= read -r line; do
        ...
        elif [[ "$SAFE_MODE" != "1" && "$KILL_SUSPICIOUS_PATH" == "1" ]] \
              && is_suspicious_path "$pid" \
...
# Disabled under SAFE_MODE so your own terminal/SSH/dev tools are never killed
# just for using CPU. Only explicit miner detection runs.
check_high_cpu() {
    [[ "$SAFE_MODE" == "1" ]] && return
